	...
	: m_create_info(create_info)
{
	...
	m_vk_image = Vulkan::CreateImage(create_info);
	m_vk_device_memory = Vulkan::AllocateDeviceMemory(m_vk_image, create_info);
}
...
{
	Vulkan::GenerateMips(m_vk_image, m_create_info.format, m_create_info.width, m_create_info.height, m_create_info.num_mips);
}
...
TextureView* Texture::GetView(const TextureViewCreateInfo& view_info)
{
	...
	if (view_info.format == TEXTURE_FORMAT_UNDEFINED)
		view_info_key.format = m_create_info.format;

	if (view_info.dimension == TEXTURE_DIMENSION_UNDEFINED)
		view_info_key.dimension = m_create_info.dimension;
	...
	if (m_views.find(view_info_key) == m_views.end())
	{
		...
		m_views.emplace(
			...
			TextureView{
				.texture = this,
				.view = Vulkan::CreateImageView(m_vk_image, view_info_key),
